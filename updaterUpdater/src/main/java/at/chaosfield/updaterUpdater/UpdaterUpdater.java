...
public class UpdaterUpdater {
	...
	public static final String apiUrl = "https://api.github.com/repos/Tylian/PackUpdate/releases/latest";
	...
	public UpdaterUpdater(String[] args) {
		if (args.length != 4) {
			System.out.println(
					"4 Parameters are required (link to online pack config, location of the local pack config inside modpack root, modpack root, location of PackUpdate jar inside modpack root)");
		}
		...
		try {
			...
			if (packUpdateEntry != null) {
				...
				System.out.println(
						"[PackUpdate Updater] Warning: could not get version of original PackUpdate. Downloading it now.");
		} catch (IOException e) {
		...
		try {
			...
			Method mainMethod = ClassLoader.getSystemClassLoader().loadClass("at.chaosfield.packupdate.PackUpdate")
	...
	public static boolean downloadPackUpdate(String path) throws IOException {
		...
		for (Object asset : jsonRelease.getJSONArray("assets")) {
			if (((JSONObject) asset).getString("name").startsWith("PackUpdate")) {
				...
				FileUtils.copyURLToFile(new URL(((JSONObject) asset).getString("browser_download_url")),
						new File(path));
	...
	private static JSONObject getJSON(String url) throws IOException {
		BufferedReader release = new BufferedReader(
